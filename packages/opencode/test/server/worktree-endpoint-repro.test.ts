...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      BIGMANCODE_EXPERIMENTAL_WORKSPACES: Flag.BIGMANCODE_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.BIGMANCODE_EXPERIMENTAL_WORKSPACES = original.BIGMANCODE_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
