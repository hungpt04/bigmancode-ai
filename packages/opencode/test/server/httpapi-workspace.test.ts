...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-bigmancode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-bigmancode-workspace": "internal",
          },
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-bigmancode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-bigmancode-workspace")
