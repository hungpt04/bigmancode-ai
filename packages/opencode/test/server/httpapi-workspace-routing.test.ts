    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          "x-bigmancode-directory": "/secret/path",
      ...
      expect(forwarded?.headers["x-bigmancode-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-bigmancode-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-bigmancode-directory", headerDir),
        HttpClient.execute,
      )
