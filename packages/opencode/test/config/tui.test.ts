...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.BIGMANCODE_CONFIG
  delete process.env.BIGMANCODE_TUI_CONFIG
...
it.instance("BIGMANCODE_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
