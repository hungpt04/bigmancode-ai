...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in bigmancode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .bigmancode/ subdirectory too)
  const candidates = [path.join(baseDir, "bigmancode.json"), path.join(baseDir, "bigmancode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".bigmancode", "bigmancode.json"), path.join(baseDir, ".bigmancode", "bigmancode.jsonc"))
  }
  ...
  // Default to bigmancode.json if none exist
  return candidates[0]
}
