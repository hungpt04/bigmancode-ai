...
describe("ModelsDevPlugin", () => {
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.BIGMANCODE_MODELS_PATH,
          disabled: Flag.BIGMANCODE_DISABLE_MODELS_FETCH,
        }
        Flag.BIGMANCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.BIGMANCODE_MODELS_PATH = previous.path
          Flag.BIGMANCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
