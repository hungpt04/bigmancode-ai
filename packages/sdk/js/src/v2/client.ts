...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-bigmancode-directory", "directory"],
    ["x-bigmancode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-bigmancode-directory")
  next.headers.delete("x-bigmancode-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-bigmancode-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-bigmancode-workspace": config.experimental_workspaceID,
    }
  }
