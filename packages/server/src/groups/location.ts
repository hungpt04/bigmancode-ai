...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-bigmancode-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-bigmancode-directory"] ? decode(request.headers["x-bigmancode-directory"]) : process.cwd())
