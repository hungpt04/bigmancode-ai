...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`BigManCode is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export BIGMANCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export BIGMANCODE_CLIENT=desktop",
    `export BIGMANCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export BIGMANCODE_SERVER_PASSWORD=${shellEscape(password)}`,
